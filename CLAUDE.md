...
- `desk/shadow.py` — P6 entry ledger, natural-exit marks, MFE/MAE, expiry settlement, hit-rate/P&L.
...
## P6 shadow execution ledger

`journal/experiments/options_shadow_ledger_v1.json` freezes the accounting schema.
`-m desk.shadow --mark-request` requests only stored instrument IDs for open SHADOW
candidates. The Codex skill fetches `get_option_quotes` and runs `--finalize-marks`;
the deterministic finalizer buys original short legs at ask, sells original long legs
at bid, subtracts fixed round-trip costs, and appends `shadow_marks.jsonl`. Marks are
informational and never close a position. Canonical exits remain expiration intrinsic
settlement. See `journal/research/options_execution_ledger_reference.md` for formulas
and the Optopsy, OptionLab, QuantLib, and LEAN design review.
...
- Tests: `.venv\Scripts\python.exe -m pytest` (66/66 after P6)
